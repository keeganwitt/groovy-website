...
- Build the affected site target locally (`:site-user:webzip` and/or
  `:site-dev:webzip`) before opening a PR. Check the deadlinks report.
- Don't merge speculative or date-sensitive content directly to
  `asf-site` — see the branch warning above.

## What *not* to do

- **Don't push to `asf-site` without the change being ready to go live.**
- Don't reformat content outside the lines your change actually touches.
  Drive-by reformatting hides the real change and is rejected.
- Don't invent JIRA issue numbers, API symbols, version numbers, or
  release dates; verify each before writing it into the site.
- Don't commit generated artefacts: `build/` directories, `*.cache` files
  under `site/src/site/blog/`, IDE files, etc.
- Don't introduce new build-time dependencies without discussion — every
  new dependency needs a license review.

## Skills

Task-specific guidance lives under [`.agents/skills/`](.agents/skills/),
each in its own directory with a `SKILL.md` describing when to use it,
the recurring failure modes for that area, and a validation checklist.
Load the relevant skill *before* writing or modifying content in its
area — the skill is more focused than this file.

| Skill | Use for |
|---|---|
| [`release-notes`](.agents/skills/release-notes/SKILL.md) | Drafting or updating a per-release `groovy-X.Y.adoc` page under `site/src/site/releasenotes/` |

Additional skills will be added as recurring tasks emerge (blog posts,
documentation pages, generator changes, ...).
