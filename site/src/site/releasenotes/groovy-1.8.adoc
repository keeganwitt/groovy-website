...
*_Enhanced DSL Support_*

- Command chains allow method calls to read like natural English sentences, making DSLs more expressive.

*_Runtime Performance_*

- Significant optimisations for integer arithmetic and direct method calls.

*_Concurrent & Parallel Execution_*

- GPars 0.11 bundled with the distribution: Actors, Dataflow, Fork/Join, Map/Reduce, and more.

*_Functional Programming_*

- Closure composition, trampoline (stack-safe recursion), memoization, and currying improvements.

*_First-Class JSON Support_*

- `JsonSlurper` and `JsonBuilder` provide seamless JSON parsing and production.

*_Compile-Time Metaprogramming_*

- New AST transforms: `@Log`, `@Field`, `@AutoClone`, `@AutoExternalizable`, `@ThreadInterrupt`, `@TimedInterrupt`, `@ConditionalInterrupt`, `@ToString`, `@EqualsAndHashCode`, `@TupleConstructor`, and more.
...
The http://www.gpars.org/[GPars] project offers developers new
...
http://www.gpars.org/[GPars website], read the
...
features, has a
http://melix.github.io/blog/2010/11/30/upcoming_groovy_goodness_automatic_thread.html[nice blog post]
covering those code interruption transformations.
