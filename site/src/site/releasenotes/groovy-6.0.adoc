...
- Sandboxing without gaps: `SecureASTCustomizer` restrictions now also cover constructors, initializer blocks, field initializers and authored code relocated into generated or synthetic members; embedders can disable `@ASTTest` when compiling source they don't control.
