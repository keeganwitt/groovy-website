...
Gathers high-level concurrency — agents, actors, dataflow, channels and parallel collections — into one coherent package, so you pick the right tool for the job instead of hand-rolling threads, and it's callable from Java too.
...
| `groovy.indy.callsite.cleaner.inline`
| `false`
| `true` performs inline-cache cleanup on the calling thread instead of
the background `PIC-Cleaner` daemon (useful for tests and deterministic
