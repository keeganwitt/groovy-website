...
- <<g5-iterator-ext,Lazy iterator methods>> offering stream-like behavior *without the overhead*.
- New utilities beyond the standard JDK for richer out-of-the-box functionality.

*_Enhanced AST Transforms_*

- New <<g5-operatorrename,`@OperatorRename` transform>> for seamless operator overloading in third-party libraries.
...
- Support for <<g5-jep512,JEP-512>> compact source files and instance main methods.

*_Modern Web Standards_*

- Enhanced <<g5-jakarta,web content creation>> with both Jakarta and legacy Javax support.
...
unless annotated with gapi:groovy.transform.Field[@Field], in which case they become field definitions of the implicit class.
...
There is a new gapi:groovy.transform.OperatorRename[@OperatorRename] AST transform
...
for a Groovy or Java method. Such methods annotated with gapi:groovy.transform.NamedParam[@NamedParam] allow
...
such annotations already when using the gapi:groovy.transform.NamedVariant[@NamedVariant] annotation or for records.
With Groovy 5, the `@NamedParam` annotation is now also added automatically when using
gapi:groovy.transform.MapConstructor[@MapConstructor] (used by gapi:groovy.transform.Immutable[@Immutable] and gapi:groovy.transform.Canonical[@Canonical]) and the `copyWith` option for `@Immutable`.
...
so you can use classes like jdk:java.time.LocalDate[], jdk:java.time.LocalTime[], and jdk:java.time.LocalDateTime[]
...
Groovy 5 provides equivalent methods on gapi:groovy.util.Iterators[Iterators] for infinite Iterators.
...
Groovy 5 adds some extra options with `Iterators.combine` and gapi:groovy.util.Iterables[Iterables]`.combine` methods for generating combinations of elements from named sources including infinite sources.
...
* Some Groovy AST transform annotations, like gapi:groovy.transform.ToString[@ToString], gapi:groovy.transform.AutoClone[@AutoClone], and gapi:groovy.transform.Sortable[@Sortable] were given
...
* Groovy classes are now compiled with JEP 181 nest host and nest members
attributes, as Java has done since JDK 11. Outer classes, inner classes,
and closures within them form a nest and can access each other's private
members directly, so the synthetic access ("bridge") methods that were
previously generated to provide that access under static compilation are
no longer emitted. This should be invisible to typical users, but
bytecode-level tools, and any code relying on the synthetic accessor
methods via reflection, may need adjusting.
(link:https://issues.apache.org/jira/browse/GROOVY-10687[GROOVY-10687])
...
* Improvements have been made when mixing gapi:groovy.transform.TupleConstructor[@TupleConstructor] and traits with default values.
...
* `File` and `Path` instances now coerce to a boolean based on whether the file or path exists on the filesystem,
via a new `asBoolean` extension method. Previously, any non-null `File` or `Path` was truthy under the default
`!= null` rule. Code that used `if (file) { ... }` to test "the variable is set" rather than "the file exists"
should switch to an explicit `!= null` check. The change can also surface indirectly when a `File`-valued entry
is read from a map via `get`; in that case, prefer `containsKey` (or an explicit `!= null`) over relying on
truthiness.
(link:https://issues.apache.org/jira/browse/GROOVY-10949[GROOVY-10949])
...
[[Groovy4.0releasenotes-5.0.8]]
== Addendum for 5.0.8

* Groovy 5.0.8 bumped the ASM version so that it now will read classes
created on later JDK versions: JDK26 and 27 (currently in EA).
Less extensive testing has occurred for Groovy 5 on these
JDK versions and in particular, the Groovy Console is
known not to work on JDK26+. Switch to Groovy 6 (currently in beta) or use JDK versions up to 25
if using the Groovy Console in Groovy 5. A future Groovy 5 version is planned
to support the Groovy Console.