...
The following methods are added or enhanced: `any`, `chop`, `collectEntries`, `countBy`,
...
`min`, `minBy`, `minComparing`, `partitionPoint`, `putAt`, `reverse`, `reverseEach`, `sort`,
`sum`, `tail`, `toSet`, `withCollectedKeys`, `withCollectedValues`, and `withIndex`.
And methods like `flatten`, `transpose` are added for `int[][]`,`long[][]`, etc.
