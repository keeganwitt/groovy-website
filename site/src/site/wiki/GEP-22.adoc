...
generated trait interface as JVM-native interface statics. External
calls of both forms — `Trait.staticMethod(...)` against the trait
itself and `Impl.staticMethod(...)` against an implementing class —
are supported.
...
| 6.0.0-alpha-2 | 2026
| Public trait static methods are *declarer-bound by default* and are
promoted onto the generated trait interface as JVM-native interface
statics, so both `Trait.m()` from outside the trait and `T.m()` from
inside the trait body resolve at the JVM level. The
...
with public statics promoted onto the trait interface as JVM-native
interface statics, and the `@groovy.transform.Virtual` marker opting
into per-implementer override dispatch for callees that need it.
...
5 (2026-06-27) From 6.0.0. Public trait static methods are now promoted onto
