...
class. (See item 4 for the static-method restriction.)
...
override form.
. `T.this.*` qualifier syntax is not supported inside trait code and is
rejected at compile time (GROOVY-12104). `T.this` is Java's
enclosing-instance access for inner classes, and a trait is not an
enclosing scope of its implementer (per item 1, `this` is the
implementing instance directly), so `T.this` has no coherent meaning
here. The rejection covers both method calls (`T.this.m(...)`) and
field/property access (`T.this.x`). For normal dispatch use
`this.m(...)` (item 1); for explicit trait-anchored dispatch use
`T.super.m(...)` (item 3). Separately, the unqualified `super.m(...)`
form of item 2 is supported for instance methods only; from a
`static` trait method the trait chain is not walked and the form is
rejected at compile time (GROOVY-12105). Use `T.super.m(...)` for the
trait's own static `m`, or call the implementer-side method by name
without a qualifier. (Note: this restriction is specific to traits.
Groovy permits unqualified `super.m(...)` in `static` methods of
plain classes, where it dispatches to the actual superclass's static
method.)
...
dispatch); external `Trait.m()` for a `@Virtual` callee is therefore
routed via the implementer rather than resolved at the JVM level.
...
the trait. For a `@Virtual` callee, which is not promoted onto the
interface, `T.m()` from trait code reaches the trait's default via
the implementer-routing path.
...
| `Impl.m()` works; `Trait.m()` resolves through implementer routing (the method is not promoted onto the interface)
| Yes
...
| Static methods
| Yes (incubating). Each implementing class receives its own copy; public statics are also promoted onto the generated trait interface as JVM-native interface statics. Declarer-bound by default; per-implementer override dispatch is opt-in via `@groovy.transform.Virtual`. See <<_static_members,Static members>>.
...
because the trait chain is not walked for statics). See item 4 of
...
stackable traits_ now cross-references item 4 for the static-method
restriction; the recommended alternatives are existing documented
