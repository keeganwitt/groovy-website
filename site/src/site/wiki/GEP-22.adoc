...
`@groovy.transform.Virtual` to opt into *per-implementer override*
...
| Static methods
| Yes (incubating). Each implementing class receives its own copy. Public statics are declarer-bound by default and promoted onto the generated trait interface as JVM-native interface statics; `@groovy.transform.Virtual` opts a static into per-implementer override dispatch and is then *not* promoted to the interface (so it has no qualified `Trait.m(...)` form). See <<_static_members,Static members>>.
...
`@groovy.transform.Virtual`; the default for public trait statics is
...
JVM level. The
`@groovy.transform.Virtual` marker opts back into per-implementer
...
with the `@groovy.transform.Virtual` marker opting into per-implementer
...
4 (2026-06-27) From 5.0.7. Added `@groovy.transform.Virtual`, a marker for
...
7 (2026-06-28) Added a "_When to use `@Virtual`_" rationale to _Static
members_ (template-method-like overridable static defaults, with a NOTE
on why an instance method, a `static` field, or an abstract contract do
not fit), and a phase framing (defining vs weaving vs using a trait, and
