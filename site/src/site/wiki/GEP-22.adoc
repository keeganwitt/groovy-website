...
*Type*:: Feature
*Status*:: Final
*Comment*:: Reflects trait semantics as of Groovy 4.0.32, plus the `@Anchored` opt-in declarer-bound dispatch marker (GROOVY-12093) and the trait-body qualifier-syntax tightening rejecting `T.this.*` (GROOVY-12104) and unqualified static `super.m(...)` (GROOVY-12105)
*Leader*:: Cédric Champeau
*Created*:: 2026-05-06
*Last modification*:: 2026-06-25
****
...
5 (2026-06-25) Tightened trait-body qualifier syntax. Added item 4
to _this, super, and stackable traits_ rejecting `T.this.*`
(GROOVY-12104; previously produced a VerifyError on 4.x and a
ClassCastException on 5.x/6.x) and rejecting unqualified
`super.m(...)` from a `static` trait method (GROOVY-12105; previously
threw `MissingMethodException` because the trait chain is not walked
for statics). Item 2 of the same section now cross-references item 4
for the static-method restriction. The recommended alternatives are
existing documented features: `this.m(...)` for normal dispatch and
`T.super.m(...)` for explicit trait-anchored dispatch.
