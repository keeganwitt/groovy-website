...
:draft: true
:keywords: groovy, deep learning, apache commons, phonetics, pytorch, tensorflow, codecs, word2vec, djl, deeplearning4j
...
* Give you insight into semantic similarity algorithms powered by machine learning and deep neural networks using technologies like PyTorch, Tensorflow, and Word2vec
...
We can swap to a https://fasttext.cc/[FastText] model, simply by switching to that model:
...
We used https://huggingface.co/fse/fasttext-wiki-news-subwords-300[this model] which has
...
Instead of look for nearest words or the similarity measure, we could return the actual word embeddings and then visualise
...
to load and use the https://research.google/pubs/universal-sentence-encoder/[USE] https://www.kaggle.com/models/google/universal-sentence-encoder[model].
...
Looking at these results, if we were doing a production ready game, we'd just pick ConceptNet, and
we'd probably look for an English only model since the multilingual one takes the longest of all 5
