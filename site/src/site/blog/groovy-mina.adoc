...
* `sessionClosed` closes the channel. The `for await` loop reading from it will then exit
...
* `async { }` and `for await` flatten the per-session loop, with virtual threads doing the
parking on blocking I/O.
