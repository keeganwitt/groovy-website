...
too, and not merely for tidiness -- it now *fails the build*, because build-initializing Groovy's
call-site cache would place its `PIC-Cleaner` thread into the image heap.
