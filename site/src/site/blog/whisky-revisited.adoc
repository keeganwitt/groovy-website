...
Paul King
:revdate: 2025-05-02T11:30:00+00:00
:keywords: whisky, whiskey, groovy, kmeans, clustering, apache echarts
:description: This post takes a first look at the Underdog and Matrix Groovy-powered dataframe libraries.
...
* https://groovy.apache.org/blog/using-groovy-with-apache-wayang[Apache Wayang's cross-platform machine learning] supporting native and Apache Spark&trade; data processing platforms
...
Let's explore the same case study, but this time, taking a first look at two new Groovy-powered dataframe libraries:
...
Underdog has the following modules: underdog-dataframe, underdog-graphs, underdog-plots, underdog-ml, and underdog-ta. We'll use all but the last of these.
...
First, we'll load our CSV file into an Underdog dataframe (removing a column we don't need):
...
When data has many dimensions, understanding the relationship between the columns can be hard.
We can look at a correlation matrix to help us understand whether there is any redundant data,
e.g. are _Sweetness_ and _Honey_, or _Tobacco_ and _Smoky_, two measure of the same thing
or different things.
...
We can go and change our clustering algorithm, e.g. `ml.clustering.agglomerative(data, nClusters: 3)`,
...
The Matrix project consists of the following modules: matrix-core, matrix-stats, matrix-datasets, matrix-spreadsheet, matrix-csv, matrix-json, matrix-xcharts, matrix-sql, matrix-parquet, matrix-bigquery, matrix-charts, and matrix-tablesaw.

While new, Matrix does build upon common JVM data science libraries, like Tablesaw and Apache Commons Math.
For certain functionality, like clustering and dimension reduction, Matrix works well with libraries like Smile.
...
methods. We want our data as numbers, and some of the functionality we'll use, e.g.
the radar plot, assumes our data is normalized (values between 0 and 1).

Rather than using `convert` or the normalization methods, here we'll show off the `apply`
...
NOTE: If `matrix-xchart` doesn't have the functionality you are after, considering
...
We can examine the cluster allocation using groovy-ginq functionality, which works well with Matrix:
...
Let's project onto 2 dimensions and place the projected coordinates back into the matrix:
...
Matrix doesn't have a correlation heatmap plot out of the box, but it does have heatmap plots,
