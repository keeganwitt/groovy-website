...
Where `all` waits for _every_ task, `any` returns when _any_ task completes — a race:
...
[source,groovy]
----
def result = await Single.just('hello')
----
...
`asyncFun()` return futures that work naturally with `await` and the `Awaitable`
...
*12/Jul/2026*: Reframe the GPars section around the built-in `groovy.concurrent` package.
****
