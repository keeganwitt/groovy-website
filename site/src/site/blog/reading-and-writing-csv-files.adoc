...
:updated: 2026-08-26T10:00:00+00:00
...
image:img/csv_typed.png["Reading a CSV file by hand with split versus reading it into records with Groovy's CsvSlurper",width=800]
...
Under the covers, `groovy-csv` currently builds on the CSV support in
https://github.com/FasterXML/jackson-databind[Jackson Databind], which
we'll be looking at directly later in this post. That is an
implementation detail rather than part of the module's API, and it
stays mostly out of sight, but a few hints reveal it here and there —
most visibly in the annotations recognised when reading and writing
typed objects. In theory the underlying library could change, though
that happens only rarely, and never without considering the porting
...
This is one of those places where the Jackson underpinnings show
through: `@JsonProperty` is a Jackson annotation. We only need it for
the two components whose names don't match their column, and `stage`
arrives as an `int` without any help. We'll meet the same annotations
again in the Jackson Databind section later, but here we don't
assemble a `CsvMapper` and a `CsvSchema` ourselves. Conversion isn't
limited to numbers, `java.time` types work too, with `@JsonFormat`
where the pattern isn't the default:
...
Writing typed objects back out is `CsvBuilder.toCsv(data, type)`. If
the column order matters, `@JsonPropertyOrder` on the type pins it
explicitly. Without it, columns follow Jackson's own ordering, which is
alphabetical; from 6.0.0-beta-4 onwards records instead keep their
component declaration order
(https://issues.apache.org/jira/browse/GROOVY-12296[GROOVY-12296]),
while other classes stay alphabetical. The annotation gives the same
result either way.

=== Revisiting the summary

Now that our rows are records, we can revisit the multiple wins summary
from earlier. The grouping key becomes the interesting part of the line
rather than a column index, and `countBy` does in one step what
`groupBy` followed by a size mapping did before, so the `bySize` helper
has no equivalent here:

[source,groovy]
----
def multipleWins = { Closure select -> cyclists
    .countBy(select)
    .findAll { it.value > 1 }
    .sort { -it.value }
    .entrySet()
...
println 'Multiple wins by country:\n' + multipleWins{ it.country }
println 'Multiple wins by rider:\n' + multipleWins{ it.full() }
println 'Multiple wins by team:\n' + multipleWins{ it.team }
----

The output is identical to the earlier version, but nothing in the
processing depends any more on which position a column happens to
occupy in the file.

image:img/csv_summary.png["The multiple wins summary rewritten with records and countBy, alongside its output",width=800]

Groovy's `groovy-ginq` module is worth a mention here too. GINQ can
group by several keys at once, so the team and country travel along
with the rider, and a second aggregate comes with them in the same
pass:

[source,groovy]
----
def repeatWinners = GQ {
    from c in cyclists
    groupby c.full() as rider, c.team as team, c.country as country
    having count() > 1
    orderby count() in desc, rider
    select rider, team, country, count() as wins, max(c.stage) as lastWin
}
repeatWinners.each {
    println "$it.wins wins  $it.rider ($it.team, $it.country), last on stage $it.lastWin"
}
----
...
[source]
----
3 wins  Tadej Pogačar (UAE Team Emirates, 🇸🇮), last on stage 17
3 wins  Wout van Aert (Jumbo-Visma, 🇧🇪), last on stage 20
2 wins  Jasper Philipsen (Alpecin-Deceuninck, 🇧🇪), last on stage 21
2 wins  Jonas Vingegaard (Jumbo-Visma, 🇩🇰), last on stage 18
----

Doing that with closures means holding on to a representative element
for each group and re-extracting the other columns from it.
...
.Update history
****
*25/Jul/2022*: Initial version. +
*26/Aug/2026*: Version 2: added a section on Groovy 6's `groovy-csv` module — `CsvSlurper`, `CsvBuilder`, RFC 4180 quoting, and typed `parseAs` reading straight into records; revisited the multiple wins summary using records with `countBy`, and with a `groovy-ginq` query grouping by several keys at once.
****
