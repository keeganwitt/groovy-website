...
Writing typed objects back out is `CsvBuilder.toCsv(data, type)`.
Records are written in component declaration order and other classes
alphabetically. If the order matters, `@JsonPropertyOrder` on the type
pins it explicitly; records came out alphabetically too in Groovy 6 beta versions,
and the annotation gives the same result either way.
