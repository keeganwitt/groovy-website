...
def fileOperation(URI uri, boolean random) {
    ...
    var mapX = new MapOperator<>(tuple -> (float[]) tuple.field0, Tuple, float[]) // <3>
    var mapY = new MapOperator<>(tuple -> (Integer) tuple.field1, Tuple, Integer) // <3>

    if (random) {
        Random r = new Random()
        var randomOperator = new SortOperator<>(e -> r.nextInt(), String, Integer) // <4>
...
<2> `mapOperator` converts a line into a Tuple containing our float[] data and an Integer label
