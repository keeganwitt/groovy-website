...
* `balance` is pure _modulo logging_. The checker's `allows` option
declares a small effect lattice (`LOGGING`, `METRICS`, `IO`,
`NONDETERMINISM`) and tolerates calls in those categories from any
`@Pure` method in scope — graded effects, in the cats-effect sense,
but the verification work is in the checker rather than the type.
