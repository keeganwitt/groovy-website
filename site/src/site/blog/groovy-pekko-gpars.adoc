...
:description: This post looks at using Apache Pekko Actors and Groovy 6's built-in groovy.concurrent Actors.
...
image:img/pekko_groovy6_actors.png["The three-actor hello world system, and the whole bot actor in Groovy 6",width=800]
...
This example uses Groovy 6.0.0-beta-2 and Pekko 1.7.0.
...
it sends the payload to the greeter, telling it to reply to a newly created _bot_.
...
The bot keeps its counter in a plain private field. Since every call is
serialized through its own actor, incrementing it is safe:
...
The output is the same as before, and there is nothing to shut down --
when the script ends, so does the JVM. Compare the `HelloWorldBot` here with
...
*12/Jul/2026*: Update to Pekko 1.6.0 and Groovy 6; replace the GPars example with the built-in `groovy.concurrent` actors (GPars retained as a legacy reference). +
*22/Aug/2026*: Update to Pekko 1.7.0 and Groovy 6.0.0-beta-2; add an `@ActiveObject` version of the example.
****
